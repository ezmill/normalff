	...
	this.rgbShader = {
		...
		fragmentShader: [
			...
			"void main() {",
				...
				// "vec2 offset = amount * vec2( sin(time), cos(time) );",
				"vec2 offset = amount * vec2( mouse );",
				"vec4 cr = texture2D(texture, vUv + offset);",
				"vec4 cga = texture2D(texture, vUv);",
				"vec4 cb = texture2D(texture, vUv - offset);",
				// "vec3 col2 = vec4(mix(cr.r, cb.r, cos(time)), mix(cr.g, cb.g, sin(time)), mix(cr.b, cb.b, cos(time)), 1.0).rgb;",
				"vec3 col2 = vec4(cr.r, mix(cr.g, cb.g, sin(time)), cb.b, 1.0).rgb;",
				"vec2 q = vUv;",
			    "vec2 p = -1.0 + 2.0*q;",
